...
class VAE(BaseDisentangler):
    ...
    def __init__(self, args):
        ...
        self.optim_G = optim.Adam(self.model.parameters(), lr=self.lr_G, betas=(self.beta1, self.beta2), weight_decay=self.latent_reg)
    ...
    def train(self):
        while not self.training_complete():
            ...
            vae_loss_sum = 0
            snd_iter = iter(self.data_loader)
            for internal_iter, (_, x_true1, label1) in enumerate(self.data_loader):
                losses = dict()
                x_true1 = x_true1.to(self.device)
                label1 = label1.to(self.device)
                _, x_true2, label2 = next(snd_iter)
                ...
                self.optim_G.step()
                if not self.log_save(input_image=x_true1, recon_image=params['x_recon'], loss=losses):
    ...
    def test(self):
        ...
        for _, x_true, label in self.data_loader:

...
from common.corr_utils import sigma_correlation, filter_function
from common.corr_utils import get_normal_distr
import common.labeling_functions as label_fns
DEBUG = False
...
class BaseDisentangler(object):
    def __init__(self, args):
        ...
        ## Apply a potential filter function on top of the correlation function 
        # (e.g., limit the datasupport by assigning zero weights to some points. The resulting function will be 
        # stored in correlation_function_filter
        self.filter_fn = None
        if args.filter_fn is not None:
            if args.filter_fn == "filter_function":
                if correlation_function is None:
                    correlation_function_filter = filter_function # Only filter. No correlation.
                else:
                ...
                if args.overwrite_cor_list and (args.corr_feature1 is not None) and (args.corr_feature2 is not None):
                    ...
                    # CAUTION: Factor pairs hardcoded for 3dshapes since dataset is not loaded at this point
                    pairs = [[0,1], [4, 5], [2,3], [0, 2], [1, 3], [1, 5], [3, 5], [2, 4], [0, 5], [1, 4], [1, 2], [3, 0], [3, 4], [0, 4], [2, 5]]
                correlation_function_filter = get_normal_distr(args.sigma_corr, args.n_cor,
        ...
        if args.labeling_fn is None:
            use_label_fn = None
        else:
            use_label_fn = getattr(label_fns, args.labeling_fn)
            args.num_classes = label_fns.labels_num_classes_dict[args.labeling_fn]
        
        # Finally initialize the dataloader with the corresponding functions and args.
        self.data_loader = get_dataloader(args.dset_name, args.dset_dir, args.batch_size, args.seed, args.num_workers,
                                          ...
                                          not args.test, resample_fn = correlation_function_filter, oversampling_factor = args.oversampling_factor,
                                          labeling_fn=use_label_fn)
        ...
        if not args.test:
            ...
            self.pbar = tqdm(total=self.max_iter, position=0, mininterval=60)
        else:
            self.pbar = tqdm(self.num_batches, position=0, mininterval=60)
        ...
        #print(args.traverse_l, args.traverse_c)
    ...
    def log_save(self, **kwargs):
        ...
        if self.evaluation_metric and is_time_for(self.iter, self.evaluate_iter):
            new_results = evaluate_disentanglement_metric(self, metric_names=self.evaluation_metric)
            # Store "best" checkpoint if eval_factor_vae_metric is either not computed or it is improved over the last checkpoint.
            if "eval_factor_vae_metric" not in self.evaluate_results or (new_results["eval_factor_vae_metric"] > self.evaluate_results["eval_factor_vae_metric"]):
                self.save_checkpoint(ckptname='best')
            self.evaluate_results = new_results
            self.evaluate_results["num_iter"] = self.iter
            if os.path.isfile(os.path.join(self.train_output_dir, "eval_results.json")):
                prev_res = json.load(open(os.path.join(self.train_output_dir, "eval_results.json"), "r"))
            else:
                prev_res = []
            prev_res.append(self.evaluate_results)
            json.dump(prev_res, open(os.path.join(self.train_output_dir, "eval_results.json"), "w"))
    ...
    def visualize_traverse(self, limit: tuple, spacing, data=None, test=False, z_dirmat = None):
        """
            by default, for z the unit directions are traversed. If z_dirmat is set to a (z_latent, num_dirs)
            matrix, these directions are used as unit directions.
        """
                ...
                for zid in range(num_dims):
                    for val in interp_values:
                        ...
                        if z_dirmat is not None:
                            ...
                            latent[:, zid] = latent_orig[0, zid] + val
                        sample = self.decode(latent=latent, labels=label_orig).detach()
                        samples.append(sample)
                        gifs.append(sample)
